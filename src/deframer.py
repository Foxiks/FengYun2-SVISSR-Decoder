import Levenshtein, argparse, time, io
...
def main(input_file, sync_marker, sync_buffer, mask, out, out_filename):
    bit_array=''
    k=0
    n=0
    while True:
...
if(__name__=='__main__'):
    ...
    with open('fy2_mask.bin', 'rb') as file:
        mask=int.from_bytes(file.read(44356), byteorder='big', signed=False)
    main(input_file=f, sync_buffer=sync_buffer, sync_marker=sync_marker, mask=mask, out=out_file, out_filename=outfile)
